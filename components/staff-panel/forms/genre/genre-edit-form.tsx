...
function GenreEditForm({ isEditDialogOpen, setIsEditDialogOpen, values }: any) {
  ...
  async function onSubmit(data: z.infer<typeof GenreSchema>) {
    ...
    try {
      const { data: genreData } = await axios.patch(
        `/genres/${values?.uuid}`,
        formData,
        {
          headers: {
      ...
      handleAxiosError(error, "An error occurred while update the genre.");
    }
  }
  return (
    <Dialog
