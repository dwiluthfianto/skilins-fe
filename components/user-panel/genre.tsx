...
export default function Genre({ genre }: { genre: string }) {
  ...
  const { genre: dataGenre } = useGenreByName(genre);
  ...
  return (
    <div className='space-y-4'>
      <div className='flex flex-col items-start justify-between gap-4 md:items-center md:flex-row'>
        <div>
          <p className='text-4xl font-bold'>{dataGenre?.name}</p>
          <p className='mt-2 text-sm'>{dataGenre?.description}</p>
        </div>
      </div>
